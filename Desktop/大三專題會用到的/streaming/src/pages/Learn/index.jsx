...
import {Wholee,Chating,Center,Left} from './style'
...
export default class Learn extends React.Component {
	...
	componentDidMount () 
	{
	    var JSWebrtc=
	    {
	    	Player:null,VideoElement:null,CreateVideoElements:function()
	    	{
	    		var elements=document.querySelectorAll(".jswebrtc");
	    		for(var i=0;i<elements.length;i++){new JSWebrtc.VideoElement(elements[i])}
	    	},
	    	...
	    	ParseUrl:function(rtmp_url)
    		{
				...
				if(!port)
				{
					...
					{port=443}
					else if(schema==="rtmp"){port=1935}
					else if(schema==="webrtc"||schema==="rtc"){port=1985}
				}
    			var ret={url:rtmp_url,schema:schema,server:a.hostname,port:port,vhost:vhost,app:app,stream:stream};
    			JSWebrtc.FillQuery(a.search,ret);return ret
    		},
    			...
    			return new Promise(function(resolve,reject){
					...
					xhr.onreadystatechange=function(){
						if(xhr.readyState===4&&(xhr.status>=200&&xhr.status<300)){
							var respone=JSON.parse(xhr.responseText);xhr.onreadystatechange=new Function;xhr=null;resolve(respone)
						}
					};
    	...
    	JSWebrtc.VideoElement=function()
    	{
    		...
    		var VideoElement=function(element)
    		{
    			...
    			addStyles(this.container,{display:"inline-block",position:"relative",minWidth:"800px",minHeight:"800px"});
    			this.video=document.createElement("video");
    			this.video.width=960;this.video.height=540;
    			...
    			{
    				try{options[option]=JSON.parse(element.dataset[option])}
    	...
    	JSWebrtc.Player=function()
    	{
    		...
    		Player.prototype.startLoading=function()
    		{
    			...
    			_self.pc.createOffer().then(function(offer)
				{
					return _self.pc.setLocalDescription(offer).then(function(){return offer})}
						...
						return new Promise(function(resolve,reject){
							...
							for(var key in _self.urlParams.user_query){
								if(key!="api"&&key!="play"){url+="&"+key+"="+_self.urlParams.user_query[key]}
							}
					...
					}).catch(function(reason){throw reason});if(this.autoplay){this.play()}
    		};
    		...
    		Player.prototype.pause=function(ev)
    		{
    			...
    			this.isPlaying=false;this.paused=true;
		...
		var url = 'webrtc://140.118.109.106/:1234/live/livestream';
		var player = new JSWebrtc.Player(url, { video: video, autoplay: true, onPlay: (obj) => { console.log("start play") } });	       
	}
	
	render() {
		return (
			<Wholee className='container-fluid g-0'>
				{/*<Side/>*/}
				{/*	<VideoBox>
					<MyFrame  src="https://www.youtube.com/embed/36YnV9STBqc" 
				...
				<Center className='col-7'>{/*影片&其他資訊?*/}
					<video id="video-webrtc" controls className="" style={{"height":"449px"}} ></video>
				</Center>
				<Chating className='col-3'> {/*聊天室*/}
					聊天室
				</Chating>
